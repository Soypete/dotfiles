...
- **Skill not in `permission.skill`** — an unlisted skill falls through to OpenCode's default (`ask`), so it prompts on every use. Add it to the allowlist in `opencode.json`.
